...
def test_ontological_proof_structure(decls: dict, node_map: dict) -> None:
    ...
    # 2. Verify exact substantive axiom set: {AxGlobalGround, AxRealityGrounding, AxIntentionalChoice, explanatory_adequacy}
    subst = sorted(set(ax.rsplit(".", 1)[-1] for ax in npg_footprint if (registry.get(ax.rsplit(".", 1)[-1]) or {}).get("tag") in ("SEM", "META")))
    expected_subst = ["AxGlobalGround", "AxIntentionalChoice", "AxRealityGrounding", "explanatory_adequacy"]
    ...
    # 3. Verify Lean proof body directly: no dead bindings, explicit 7-step chain
    ...
    assert "rational_act_carries_personal s' p' hAct'" not in npg_code, "Regression FAILED: unused rational_act_carries_personal call must be removed from necessary_personal_ground_derived"
    ...
    assert "established_will_reality" in npg_body, "Step 1: Must obtain actual Person witness from established_will_reality"
    assert "step1_necessary_truth_exists" in npg_body, "Step 2: Must obtain necessary truth from step1_necessary_truth_exists"
    assert "T7_necessaryReality" in npg_body, "Step 3: Must obtain necessary ground from T7_necessaryReality"
    assert "AxRealityGrounding" in npg_body, "Step 4: Must instantiate AxRealityGrounding on EntityOf s"
    assert "atom_cannot_ground_person" in npg_body, "Step 5: Must refute impersonal atom via atom_cannot_ground_person"
    assert "GroundsPersonalReality" in npg_body, "Step 6: Must construct GroundsPersonalReality"
    assert "⟨g, hgNec, hgPersonal, hNotImp⟩" in npg_body, "Step 7: Must construct NecessaryPersonalGround directly"
    print("  ✓ Test 2 passed: Explicit 7-step proof verified with zero dead bindings and proper directionality.")
