...
- §5.4 disclosed-cost template (TWO tagged SEM postulates `Good`/`Evil`): the price
  sheet for any positive close of GAPMAP frontier F3, with the consistency witness
  `moral_pole_postulate_is_not_a_consequence` proving the poles are never forced.
...
witnesses the gap instead.

## Footprint discipline
M_amoral theorems stay `{}` (local defs + definitional `DeonticOpposition` /
`Incompatible`). The §5.4 `ClaimsMoralRectitude`/`stance_exhibits_moral_poles` carry the
price IN the footprint: `{Evil, Good, Initiates, Means, State, Subject}`. `Good`/`Evil`
list themselves in their own `#print axioms`; `moral_pole_postulate_is_not_a_consequence`
is vocabulary-only `{Initiates, Means, State, Subject}`.
-/
...
-- ===========================================================================
-- §5.4 Disclosed-cost positive template (approved 2026-09-24)
-- ===========================================================================

/--
Tag: SEM
Good: the positive moral pole of a subject's practical action — a declared semantic datum.

The poles are genuinely additional, never forced: the consistency witness
`moral_pole_postulate_is_not_a_consequence` interprets Good and Evil as false everywhere
...
Philosophical cost: declaring a moral pole (Good/Evil) is a substantive semantic commitment
— the price sheet of the moral-frontier bridge (F3): epistemic agential normativity is
machine-separated from practical bindingness (`M_amoral`, C175), so bindingness cannot be
read off the relying structure and must be postulated with the price written out loud.
-/
axiom Good : Subject → Prop → Prop
...
/--The moral-judicative stance: the subject performs its act and means its action's Good and
  Evil — mirroring the normative-judicative stance with declared poles.
  Footprint: `{Evil, Good, Initiates, Means, State, Subject}` (the price is written into
...
/--The stance exhibits both moral poles: moral rectitude towards an action means the subject
  ...
  Footprint: `{Evil, Good, Initiates, Means, State, Subject}`. -/
...
/--The moral poles are genuinely additional: interpretable as false everywhere with the
  epistemic reality-hook intact — never forced by the relying structure.
